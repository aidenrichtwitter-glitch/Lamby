...
import {
  ...
  runGrokEvolutionCycle, ensureEvolutionPlan, runFullEvolutionCycle,
  type EvolutionPlan, type EvolutionCycleResult, type FullEvolutionCycleResult,
} from '@/lib/evolution-bridge';
import { ensureEvolutionSandbox, EVOLUTION_SANDBOX_NAME } from '@/lib/project-manager';
...
const PatternAnalysis: React.FC = () => {
  ...
  const runEvolutionCycle = useCallback(async () => {
    ...
    setCycleStatus('Starting full 22-step evolution cycle...');

    try {
      let context = `=== PROJECT CONTEXT ===\nLamby — React + TypeScript + Vite desktop IDE with Electron.\n`;
      try {
      ...
      const result = await runFullEvolutionCycle({
        ...
        onStatus: (status) => setCycleStatus(status),
      ...
      const applied = result.blocks.filter(b => b.status === 'applied').length;
      setCycleStatus(`Done — ${succeeded}/22 steps, ${applied} files applied, L${result.newLevel}`);
      setActivePhase(null);
      fetchData();
    } catch (e: any) {
  ...
  const runBrowserEvolution = useCallback(async () => {
    setCycleRunning(true);
    setCycleResult(null);
    setGrokStream('');
    setShowGrokPanel(true);
    setCycleStatus('Preparing full 22-step browser evolution...');

    try {
      const automation = (window as any).__grokBrowserAutomation;
      if (!automation) throw new Error('Browser automation not available — open AI Bridge tab first and load Grok');

      setCycleStatus('Ensuring evolution sandbox...');
      const sandbox = await ensureEvolutionSandbox();

      setCycleStatus('Checking evolution plan...');
      await ensureEvolutionPlan(true);
      ...
      const result = await runFullEvolutionCycle({
        ...
        model: 'grok-4',
        useBrowser: true,
        targetProject: sandbox,
        onDelta: (delta) => {
          setGrokStream(prev => {
            const updated = prev + delta;
            requestAnimationFrame(() => {
              const el = document.getElementById('grok-stream-panel');
      ...
      setCycleStatus(result.error
        ? `Browser evolution error: ${result.error}`
        : `Browser evolution complete — ${succeeded}/22 steps, ${appliedCount} files written to sandbox`
      );
      setActivePhase(null);
      fetchData();
    } catch (e: any) {
      setCycleResult({ fullResponse: '', blocks: [], planSaved: false, capabilitiesRegistered: [], newLevel: 0, error: e.message || String(e) });
      setCycleStatus(`Error: ${e.message}`);
      setActivePhase(null);
    } finally {
      setCycleRunning(false);
      ...
      <header className="border-b border-border bg-card/50 backdrop-blur-sm shrink-0">
        <div className="px-4 py-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              data-testid="button-browser-evolve"
              onClick={runBrowserEvolution}
              disabled={cycleRunning}
              className="text-[9px] px-3 py-1 rounded bg-[hsl(200_80%_45%)] text-white hover:bg-[hsl(200_80%_35%)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-1.5 font-semibold"
            >
              {cycleRunning ? (
                <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}>
                  <RefreshCw className="w-3 h-3" />
              ...
              {cycleRunning ? 'Running...' : 'Browser-Evolve'}
            </button>
      ...
      {loading ? (
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          >
            ...
            >
              <img src="/lamby-logo.png" alt="Lamby" className="w-5 h-5" />
