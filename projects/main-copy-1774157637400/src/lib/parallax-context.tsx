...
interface ParallaxContextValue extends ParallaxState {
  ...
  setRightPanelCount: (n: number) => void;
  setRightWallWidthPx: (px: number) => void;
...
export function ParallaxProvider({ children }: { children: React.ReactNode }) {
  ...
  const [rightPanelCount, setRightPanelCount] = useState(0);
  const [rightWallWidthPx, setRightWallWidthPx] = useState(0);
  ...
  return (
    <ParallaxContext.Provider value={{
      ...
      rightPanelCount, setRightPanelCount,
      rightWallWidthPx, setRightWallWidthPx,
