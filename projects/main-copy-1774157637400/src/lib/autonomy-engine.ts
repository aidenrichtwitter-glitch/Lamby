...
export async function checkGoalProgress(): Promise<AutonomyTask> {
  const start = performance.now();
...
export async function executeGoalStep(): Promise<{ task: AutonomyTask; goalResult: AutonomyReport['goalAttempted'] }> {
  const start = performance.now();
...
export async function reflectOnCycle(
  tasks: AutonomyTask[],
