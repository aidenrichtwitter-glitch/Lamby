...
const Keyboard = () => {
  ...
  const handleCharacterKey = (base: string, shifted?: string) => {
    ...
    if (isLetter(base)) {
      ...
      const shouldUppercase = (capsOn && !shiftOn) || (!capsOn && shiftOn);

      charToAdd = shouldUppercase ? base.toUpperCase() : base.toLowerCase();
    } else {
  ...
  return (
    <div className="keyboard-container">
      <textarea className="keyboard-display" value={inputText} readOnly />
          ...
          {ROW1.map((keyObj, index) => (
            ...
            >
              {"base" in keyObj && keyObj.shifted ? (
                <>
                  <span className="keyboard-key--shifted">
                    {keyObj.shifted}
                  </span>
