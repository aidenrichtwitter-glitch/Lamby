...
function fileWritePlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use("/api/calibration", async (req, res) => {
        const fs = await import("fs");
        const calPath = path.resolve(process.cwd(), ".local/calibration.json");
        if (req.method === "GET") {
          try {
            if (fs.existsSync(calPath)) {
              res.setHeader("Content-Type", "application/json");
              res.end(fs.readFileSync(calPath, "utf-8"));
            } else {
              res.setHeader("Content-Type", "application/json");
              res.end(JSON.stringify({ calibrated: false }));
            }
          } catch (err: any) {
            res.statusCode = 500;
        ...
        if (req.method === "POST") {
          try {
            ...
            if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
            fs.writeFileSync(calPath, JSON.stringify(data, null, 2));
            res.setHeader("Content-Type", "application/json");
            res.end(JSON.stringify({ success: true, calibrated: true }));
          } catch (err: any) {
            res.statusCode = 500;
            res.end(JSON.stringify({ error: err.message }));
          }
      ...
      server.middlewares.use("/api/projects/preview", async (req, res) => {
        ...
        try {
          ...
          if (previewProcesses.has(name)) {
            ...
            try {
              const existingPid = existing.process.pid;
              if (process.platform === "win32") {
                try { const { execSync: es } = await import("child_process"); es(`taskkill /pid ${existingPid} /T /F`, { stdio: "pipe", windowsHide: true }); } catch {}
              } else {
                if (existingPid && existingPid !== process.pid) {
                  try { process.kill(-existingPid, 9); } catch {}
                }
              }
  ...
  setTimeout(function() {
    ...
    try {
      ...
      const GROK_FC_TOOLS = [
        ...
        { type: "function", name: "sketchfab_search", description: "Search Sketchfab for downloadable 3D models. Returns model names, UIDs, thumbnails, vertex/face counts.", parameters: { type: "object", properties: { query: { type: "string", description: "Search query (e.g. 'dragon', 'low poly car', 'sci-fi weapon')" }, count: { type: "number", description: "Number of results (default 12, max 24)" } }, required: ["query"] } },
        { type: "function", name: "sketchfab_download", description: "Download a 3D model from Sketchfab to the user's Downloads folder and optionally open it in Blender. Uses the bridge to run PowerShell on the user's machine.", parameters: { type: "object", properties: { uid: { type: "string", description: "Sketchfab model UID from search results" }, modelName: { type: "string", description: "Human-readable model name for the filename" }, openInBlender: { type: "boolean", description: "If true, automatically import the model into Blender after download (default true)" } }, required: ["uid"] } },
      ];
      ...
      async function executeGrokFunctionCall(name: string, args: any, bridgeRelayUrl: string): Promise<string> {
        ...
        switch (name) {
          ...
          case "sketchfab_search": {
            const sfApiKey = process.env.sketchfabapi || process.env.SKETCHFAB_API_KEY || "";
            if (!sfApiKey) return JSON.stringify({ error: "SKETCHFAB_API_KEY not configured" });
            try {
              const sfCount = Math.min(args.count || 12, 24);
              const sfParams = new URLSearchParams({ type: "models", q: args.query, downloadable: "true", count: String(sfCount) });
              const sfResp = await fetch(`https://api.sketchfab.com/v3/search?${sfParams.toString()}`, { headers: { Authorization: `Token ${sfApiKey}` } });
              if (!sfResp.ok) return JSON.stringify({ error: `Sketchfab API error: ${sfResp.status} ${sfResp.statusText}` });
              const sfData = await sfResp.json();
              const models = (sfData.results || []).map((m: any) => ({
                uid: m.uid, name: m.name, viewerUrl: m.viewerUrl,
                vertexCount: m.vertexCount, faceCount: m.faceCount, animationCount: m.animationCount,
                user: m.user?.displayName || m.user?.username,
                thumbnail: m.thumbnails?.images?.find((i: any) => i.width >= 200 && i.width <= 400)?.url || m.thumbnails?.images?.[0]?.url,
                isDownloadable: m.isDownloadable,
              }));
              return JSON.stringify({ count: sfData.count || models.length, models });
            } catch (e: any) {
              return JSON.stringify({ error: e.message });
            }
          }
          case "sketchfab_download": {
            const sfApiKey2 = process.env.sketchfabapi || process.env.SKETCHFAB_API_KEY || process.env.SKETCHFAB_API_TOKEN || "";
            if (!sfApiKey2) return JSON.stringify({ error: "SKETCHFAB_API_KEY not configured" });
            const uid = args.uid;
            if (!uid || !/^[a-f0-9]{32}$/.test(uid)) return JSON.stringify({ error: "uid is required and must be a 32-char hex string" });
            try {
              const dlResp = await fetch(`https://api.sketchfab.com/v3/models/${uid}/download`, { headers: { Authorization: `Token ${sfApiKey2}` } });
              if (!dlResp.ok) {
                const errText = await dlResp.text();
                return JSON.stringify({ error: `Sketchfab download API error: ${dlResp.status}`, detail: errText });
              }
              const dlData = await dlResp.json();
              const isGlb = !!dlData.glb?.url;
              const downloadUrl = dlData.glb?.url || dlData.gltf?.url;
              if (!downloadUrl) return JSON.stringify({ error: "No downloadable GLB/GLTF format available for this model" });

              const dlUrlObj = new URL(downloadUrl);
              if (!dlUrlObj.hostname.endsWith("sketchfab.com") && !dlUrlObj.hostname.includes("amazonaws.com")) {
                return JSON.stringify({ error: "Download URL is not from a trusted Sketchfab domain" });
              }

              const safeName = (args.modelName || uid).replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 40);
              const ext = isGlb ? "glb" : "zip";
              const fileName = `${safeName}.${ext}`;
              const dlDir = "C:\\Users\\Aiden\\Downloads";
              const destPath = `${dlDir}\\${fileName}`;
              const destPathFwd = `C:/Users/Aiden/Downloads/${fileName}`;
              const doOpen = args.openInBlender !== false;
              const execUrl2 = `${bridgeRelayUrl}/api/sandbox/execute`;

              const safeUrl = downloadUrl.replace(/'/g, "''");
              const dlCmd = `powershell -NoProfile -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '${safeUrl}' -OutFile '${destPath}' -UseBasicParsing; if (Test-Path '${destPath}') { Write-Output ('DOWNLOADED: ${destPath} (' + (Get-Item '${destPath}').Length + ' bytes)') } else { Write-Output 'DOWNLOAD_FAILED' }"`;
              const dlResp2 = await fetch(execUrl2, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ actions: [{ type: "run_command", command: dlCmd, project: "__system__" }] }) });
              const dlResult = await dlResp2.json();
              const dlOutput = dlResult?.results?.[0]?.data?.output || "";
              if (dlOutput.includes("DOWNLOAD_FAILED")) return JSON.stringify({ error: "Download failed", results: dlResult });
              ...
              if (doOpen && isGlb) {
                ...
                const pyB64 = Buffer.from(pyScript).toString("base64");
                const pyDest = "C:\\Users\\Aiden\\Desktop\\blender_import.py";

                const writeScriptPs = `[System.IO.File]::WriteAllText("${pyDest}", [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String("${pyB64}"))); Write-Output "Script written"`;
                const writeB64 = Buffer.from(writeScriptPs, "utf16le").toString("base64");
                const writeCmd = `powershell -NoProfile -EncodedCommand ${writeB64}`;

                await new Promise(r => setTimeout(r, 3500));
                const writeResp = await fetch(execUrl2, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ actions: [{ type: "run_command", command: writeCmd, project: "__system__" }] }) });
                const writeResult = await writeResp.json();

                const blenderExe = "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe";
                const launchCmd = `powershell -NoProfile -Command "Start-Process -FilePath '${blenderExe}' -ArgumentList '--python','${pyDest}'; Write-Output 'Blender launched'"`;

                await new Promise(r => setTimeout(r, 3500));
                const launchResp = await fetch(execUrl2, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ actions: [{ type: "run_command", command: launchCmd, project: "__system__" }] }) });
                const launchResult = await launchResp.json();

                return JSON.stringify({ success: true, file: destPath, format: ext, blenderOpened: true, download: dlResult, scriptWrite: writeResult, blenderLaunch: launchResult });
              } else if (doOpen && !isGlb) {
                return JSON.stringify({ success: true, file: destPath, format: ext, blenderOpened: false, note: `Model downloaded as GLTF ZIP (${fileName}). Extract the ZIP and use open-in-blender task to import the .gltf file.`, download: dlResult });
              }

              return JSON.stringify({ success: true, file: destPath, format: ext, blenderOpened: false, download: dlResult });
            } catch (e: any) {
              return JSON.stringify({ error: e.message });
            }
          }
      ...
      server.middlewares.use("/api/sketchfab/search", async (req, res) => {
        ...
        const apiKey = process.env.sketchfabapi || process.env.SKETCHFAB_API_KEY || "";
        if (!apiKey) { res.statusCode = 500; res.setHeader("Content-Type", "application/json"); res.end(JSON.stringify({ error: "SKETCHFAB_API_KEY not configured" })); return; }
        try {
          const params = new URLSearchParams({ type, q, downloadable, count });
          if (cursor) params.set("cursor", cursor);
          const sfRes = await fetch(`https://api.sketchfab.com/v3/search?${params.toString()}`, {
            headers: { Authorization: `Token ${apiKey}` },
          });
          if (!sfRes.ok) { res.statusCode = sfRes.status; res.setHeader("Content-Type", "application/json"); res.end(JSON.stringify({ error: `Sketchfab API error: ${sfRes.status} ${sfRes.statusText}` })); return; }
      ...
      server.middlewares.use("/api/sketchfab/download-url", async (req, res) => {
        if (req.method === "OPTIONS") { res.setHeader("Access-Control-Allow-Origin", "*"); res.setHeader("Access-Control-Allow-Methods", "GET,OPTIONS"); res.statusCode = 204; res.end(); return; }
        if (req.method !== "GET") { res.statusCode = 405; res.end("Method not allowed"); return; }
        const url = new URL(req.url || "", `http://${req.headers.host}`);
        const uid = url.searchParams.get("uid") || "";
        if (!uid) { res.statusCode = 400; res.setHeader("Content-Type", "application/json"); res.end(JSON.stringify({ error: "uid parameter required" })); return; }
        const apiKey = process.env.sketchfabapi || process.env.SKETCHFAB_API_KEY || "";
        if (!apiKey) { res.statusCode = 500; res.setHeader("Content-Type", "application/json"); res.end(JSON.stringify({ error: "SKETCHFAB_API_KEY not configured" })); return; }
        try {
          ...
          if (!sfRes.ok) {
            ...
            res.statusCode = sfRes.status; res.setHeader("Content-Type", "application/json");
            res.end(JSON.stringify({ error: `Sketchfab download API error: ${sfRes.status}`, detail: errText }));
            return;
          }
          const data = await sfRes.json();
          res.setHeader("Content-Type", "application/json");
