...
[
  ...
  },
  {
    "type": "function",
    "name": "graph_index",
    "description": "Get the full project dependency graph with feature slices, nodes, edges, and entry points",
    "parameters": {
      "type": "object",
      "properties": {
        "project": { "type": "string", "description": "Project name" }
  ...
  },
  {
    "type": "function",
    "name": "graph_query",
    "description": "Query a specific file or feature slice in the dependency graph",
    "parameters": {
      "type": "object",
      "properties": {
        "project": { "type": "string" },
        "target": { "type": "string", "description": "File path or slice name to query" }
      },
  ...
  },
  {
    ...
    "description": "Analyze which files would be affected by changes to the specified files",
    "parameters": {
      "type": "object",
      "properties": {
        "project": { "type": "string" },
        "files": { "type": "array", "items": { "type": "string" }, "description": "File paths that will be changed" }
      },
  ...
  },
  {
    "type": "function",
    "name": "pattern_search",
    "description": "Search for files matching structural patterns (e.g. 'all React components', 'importing from @/api')",
    "parameters": {
      "type": "object",
      "properties": {
        "project": { "type": "string" },
  ...
  },
  {
    ...
    "description": "Dry-run validation of a batch of file operations (writes, search-replaces, deletes) with type-checking",
    "parameters": {
      "type": "object",
      "properties": {
        "project": { "type": "string" },
        "operations": { "type": "array", "items": { "type": "object" }, "description": "Array of operations: {type:'write',path,content}, {type:'search_replace',path,search,replace}, {type:'delete',path}" }
      },
  ...
  },
  {
    ...
    "description": "Apply a validated batch of file operations with git stash rollback point",
    "parameters": {
      "type": "object",
      "properties": {
        "project": { "type": "string" },
        "operations": { "type": "array", "items": { "type": "object" }, "description": "Same format as changeset_validate operations" }
      },
      "required": ["project", "operations"]
    }
  },
  {
    "type": "function",
    "name": "changeset_simulate",
    "description": "Simulate changes in a temporary copy: applies changes, runs type-check and build, captures errors",
    "parameters": {
      "type": "object",
      "properties": {
        "project": { "type": "string" },
        "operations": { "type": "array", "items": { "type": "object" }, "description": "Same format as changeset_validate operations" },
        "waitMs": { "type": "integer", "description": "How long to wait for dev server errors (ms, default 8000)" }
      },
      "required": ["project", "operations"]
    }
  }
...
async function executeFunctionCall(fc) {
  ...
  switch (fc.name) {
    ...
    case 'graph_index':
      return await bridgeProxy(project, [{ type: 'graph_index', project }]);
    case 'graph_query':
      return await bridgeProxy(project, [{ type: 'graph_query', project, target: args.target }]);
    case 'impact_analysis':
      return await bridgeProxy(project, [{ type: 'impact_analysis', project, files: args.files }]);
    case 'pattern_search':
      return await bridgeProxy(project, [{ type: 'pattern_search', project, pattern: args.pattern }]);
    case 'changeset_validate':
      return await bridgeProxy(project, [{ type: 'changeset_validate', project, operations: args.operations }]);
    case 'changeset_apply':
      return await bridgeProxy(project, [{ type: 'changeset_apply', project, operations: args.operations }]);
    case 'changeset_simulate':
      return await bridgeProxy(project, [{ type: 'changeset_simulate', project, operations: args.operations, waitMs: args.waitMs }]);
    default:
...
ADVANCED: GET proxy for multi-action chains (read + edit + run in one call):
  ...
  ── GRAPH INTELLIGENCE ──
    { type: "graph_index", project: "P" }  → full project dependency graph with slices, nodes, edges, entryPoints (cached by mtime)
    { type: "graph_query", project: "P", target: "src/App.tsx" }  → query a file or slice name, returns node + all connected nodes/edges
    { type: "impact_analysis", project: "P", files: ["src/api.ts"] }  → walk dependents of changed files, returns direct/transitive affected + risk score
    { type: "pattern_search", project: "P", pattern: "all React components" }  → structural pattern search (components, imports, extensions, text)

  ── CHANGESET OPERATIONS ──
    { type: "changeset_validate", project: "P", operations: [{type:"write",path:"f.ts",content:"..."},{type:"search_replace",path:"a.ts",search:"old",replace:"new"},{type:"delete",path:"x.ts"}] }  → dry-run validation of batch edits with type-check
    { type: "changeset_apply", project: "P", operations: [...] }  → apply batch edits with git stash rollback point
    { type: "changeset_simulate", project: "P", operations: [...] }  → copy project to temp dir, apply changes, run type-check + build, capture errors
