...
const LOCAL_RELAY_PORT = parseInt(process.env.RELAY_PORT || "4100", 10);
const LOCAL_RELAY_WS = `ws://localhost:${LOCAL_RELAY_PORT}/bridge-ws`;

function isLocalhostUrl(url) {
  return /^(wss?|https?):\/\/(localhost|127\.0\.0\.1|0\.0\.0\.0)(:|\/|$)/.test(url || "");
}

function probeLocalRelay() {
  return new Promise((resolve) => {
    const req = http.get(`http://localhost:${LOCAL_RELAY_PORT}/health`, { timeout: 2000 }, (res) => {
      let d = "";
      res.on("data", (c) => { d += c; });
...
let _usingLocalRelay = false;

async function resolveRelayUrl() {
  if (bridgeConfig.preferRemote) {
    _usingLocalRelay = false;
    const configured = bridgeConfig.relayUrl || CANONICAL_RELAY_URL;
    console.log(`[Bridge] Config set preferRemote — using remote: ${configured}`);
    return configured;
  }
  const localAlive = await probeLocalRelay();
  if (localAlive) {
    console.log(`[Bridge] Local relay detected on port ${LOCAL_RELAY_PORT} — using local`);
    _usingLocalRelay = true;
    return LOCAL_RELAY_WS;
  }
  _usingLocalRelay = false;
  const configured = bridgeConfig.relayUrl || CANONICAL_RELAY_URL;
  console.log(`[Bridge] Local relay not available — using remote: ${configured}`);
  return configured;
}
...
function createBridgeConnector() {
  ...
  bridgeConnector = createConnector({
    ...
    previewPort: _activePreviewPort || "0",
    ...
    onMessage: async (msg, send) => {
      ...
      if (msg.type === "sandbox-execute-request" && msg.requestId) {
        ...
        try {
          const result = await executeSandboxActions(msg.actions || [], PROJECTS_DIR, { auditLog: sandboxAuditLog, previewProcesses, activePreviewPort: _activePreviewPort, electronIpc: typeof process.send === "function" });
...
async function connectToBridgeRelay() {
  if (bridgeConnector) {
    bridgeConnector.disconnect();
  }
  const resolvedUrl = await resolveRelayUrl();
  createBridgeConnector();
  if (bridgeConnector) {
    bridgeConnector.reconnect(resolvedUrl);
    console.log(`[Bridge] connect() called (${_usingLocalRelay ? 'LOCAL' : 'REMOTE'}: ${resolvedUrl}) — waiting for relay handshake...`);
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === "/api/sandbox/execute") {
    ...
    try {
      ...
      const result = await executeSandboxActions(actions, PROJECTS_DIR, { auditLog: sandboxAuditLog, previewProcesses, activePreviewPort: _activePreviewPort, electronIpc: typeof process.send === "function" });
      sendJson(res, result);
    } catch (err) {
  ...
  if (pathname === "/api/projects/preview") {
    ...
    try {
      ...
      if (procExited && !portDetected) {
        ...
        _activePreviewPort = finalPort;
        if (bridgeConnector && bridgeConnector.setPreviewPort) {
          bridgeConnector.setPreviewPort(finalPort);
        }
  ...
  if (pathname === "/api/sketchfab/search") {
    if (req.method !== "GET") { res.writeHead(405); res.end("Method not allowed"); return; }
    const q = url.searchParams.get("q") || "";
    const count = url.searchParams.get("count") || "24";
    const apiKey = process.env.sketchfabapi || process.env.SKETCHFAB_API_KEY || process.env.SKETCHFAB_API_TOKEN || "";
    if (!apiKey) { sendJson(res, { error: "SKETCHFAB_API_KEY not configured" }, 500); return; }
    try {
      const params = new URLSearchParams({ type: "models", q, downloadable: "true", count });
      const sfRes = await fetch(`https://api.sketchfab.com/v3/search?${params.toString()}`, { headers: { Authorization: `Token ${apiKey}` } });
      if (!sfRes.ok) { sendJson(res, { error: `Sketchfab API error: ${sfRes.status}` }, sfRes.status); return; }
  ...
  if (pathname === "/api/sketchfab/download-url") {
    if (req.method !== "GET") { res.writeHead(405); res.end("Method not allowed"); return; }
    const uid = url.searchParams.get("uid") || "";
    if (!uid) { sendJson(res, { error: "uid parameter required" }, 400); return; }
    const apiKey = process.env.sketchfabapi || process.env.SKETCHFAB_API_KEY || process.env.SKETCHFAB_API_TOKEN || "";
    if (!apiKey) { sendJson(res, { error: "SKETCHFAB_API_KEY not configured" }, 500); return; }
    try {
      const sfRes = await fetch(`https://api.sketchfab.com/v3/models/${uid}/download`, { headers: { Authorization: `Token ${apiKey}` } });
      if (!sfRes.ok) { sendJson(res, { error: `Sketchfab download API error: ${sfRes.status}` }, sfRes.status); return; }
      const data = await sfRes.json();
      sendJson(res, data);
    } catch (err) { sendJson(res, { error: err.message }, 500); }
    return;
  ...
  if (pathname === "/api/bridge-reconnect") {
    ...
    connectToBridgeRelay().then(() => {}).catch(() => {});
  ...
  if (pathname === "/api/projects/import-github") {
    ...
    try {
      ...
      let repo = body.repo || "";
      let repoUrl = body.url || "";
      const targetProject = body.targetProject || body.projectName || "";
      ...
      if (!owner || !repo) { sendJson(res, { error: "Missing owner/repo or url" }, 400); return; }
      if (!/^[a-zA-Z0-9_.-]+$/.test(owner) || !/^[a-zA-Z0-9_.-]+$/.test(repo)) { sendJson(res, { error: "Invalid owner or repo name" }, 400); return; }

      const name = targetProject ? targetProject.replace(/[^a-zA-Z0-9_-]/g, "-").replace(/^[-_.]+/, "") : repo.toLowerCase().replace(/[^a-z0-9-]/g, "-");
      ...
      if (!targetDir.startsWith(resolvedProjectsDir + path.sep) || targetDir === resolvedProjectsDir) {
        sendJson(res, { error: "Invalid target project path" }, 400);
        return;
      }

      const safeRepoUrl = `https://github.com/${owner}/${repo}.git`;

      if (targetProject && fs.existsSync(targetDir)) {
        try { fs.rmSync(targetDir, { recursive: true, force: true }); } catch {}
        console.log(`[Import] Cleared existing project '${name}' for clone`);
      }

      if (fs.existsSync(targetDir) && !targetProject) {
        try {
          require("child_process").execFileSync("git", ["pull"], { cwd: targetDir, timeout: 60000, stdio: "pipe", windowsHide: true });
        } catch (pullErr) {
          sendJson(res, { error: `git pull failed: ${pullErr.message}` }, 500);
          return;
        }
        const filesWritten = countFiles(targetDir);
        sendJson(res, { success: true, projectName: name, action: "pulled", filesWritten, framework: detectFramework(targetDir) });
      } else {
        require("child_process").execFileSync("git", ["clone", "--depth", "1", safeRepoUrl, targetDir], { timeout: 120000, stdio: "pipe", windowsHide: true });
        const filesWritten = countFiles(targetDir);
        sendJson(res, { success: true, projectName: name, action: "cloned", filesWritten, framework: detectFramework(targetDir) });
      }
    } catch (err) {
      sendJson(res, { error: err.message }, 500);
    }
  ...
  if (pathname === "/api/bridge-relay-status") {
    ...
    sendJson(res, {
      ...
      usingLocalRelay: _usingLocalRelay,
      localRelayPort: LOCAL_RELAY_PORT,
      localRelayWs: LOCAL_RELAY_WS,
    });
    return;
  }
  ...
  if (pathname === "/api/browser-interact") {
    if (req.method !== "POST") { res.writeHead(405); res.end("Method not allowed"); return; }
    if (typeof process.send !== "function") {
      sendJson(res, { success: false, error: "Not running inside Electron — browser-interact requires the desktop app" }, 501);
      return;
    }
    try {
      const body = JSON.parse(await readBody(req));
      ...
      const result = await new Promise((resolve) => {
        ...
        const timeout = setTimeout(() => { process.removeListener("message", handler); resolve({ success: false, error: "Electron IPC timeout" }); }, 15000);
        process.on("message", handler);
        const { type: _dropType, requestId: _dropReqId, ...safeBody } = body;
        process.send({
          type: "browser-interact",
          requestId,
          ...safeBody,
          port: body.port || _activePreviewPort || null,
        });
      });
      sendJson(res, result);
    } catch (err) {
    ...
    const proxyReq = http.request(
      ...
      (proxyRes) => {
        ...
        if (isHtml) {
          ...
          proxyRes.on("error", () => {
            if (!res.headersSent) { res.writeHead(502); res.end("Preview proxy error while buffering HTML"); }
          });
          proxyRes.on("aborted", () => {
            if (!res.headersSent) { res.writeHead(502); res.end("Preview proxy aborted while buffering HTML"); }
          });
          proxyRes.on("end", () => {
...
(function() {
  ...
  window.addEventListener('unhandledrejection', function(event) {
    ...
    send('error', ['Unhandled Promise Rejection: ' + msg], stack);
            ...
            if (!html.includes("lamby-preview-path")) {
              html = html.replace(/<head([^>]*)>/, `<head$1>\n${previewPathFixScript}`);
            }
            if (!html.includes("lamby-console-bridge")) {
              html = html.replace(/<head([^>]*)>/, `<head$1>\n${consoleBridgeScript}`);
            }
            delete resHeaders["content-length"];
            delete resHeaders["content-encoding"];
            delete resHeaders["etag"];
            resHeaders["content-length"] = Buffer.byteLength(html, "utf-8");
    ...
    ws.on("message", async (data) => {
      try {
        ...
        if (msg.type === "execute") {
          ...
          const result = await executeSandboxActions(actions, PROJECTS_DIR, { auditLog: sandboxAuditLog, onActionResult, previewProcesses, activePreviewPort: _activePreviewPort, electronIpc: typeof process.send === "function" });
