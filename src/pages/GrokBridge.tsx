...
const BROWSER_SITES = [
  ...
  { id: 'relay', name: 'Relay', url: 'http://localhost:4100', icon: '🌉' },
];
...
function injectMethodologyAndFailover(template: string, relayBase: string, allTunnelUrls: string[]): string {
  let t = template.replace(/\{\{RELAY_BASE\}\}/g, relayBase);
  t = t.replace(/\(Replace \{\{RELAY_BASE\}\}[^)]*\)\n?/, '');
  const methodologyBlock = `
=== CORE METHODOLOGY (follow this EXACTLY for EVERY task) ===

PHASE 1 — ORIENT (mandatory first step, no exceptions):
  Call: GET ${relayBase}/api/grok-orient?project=PROJECT&goal=YOUR_TASK_DESCRIPTION
  This returns: file tree, git status, git diff, recent commits, open windows, screenshot, relevant skills, recent errors, and all endpoint URLs.
  READ the entire response. Understand the current state BEFORE planning anything.
  The orient call also registers your goal in the step tracker when you include the goal parameter.
  Then: POST ${relayBase}/api/grok-goal with {"goal": "YOUR_GOAL", "steps": ["step1", "step2", ...]}
  This registers your detailed plan so progress is tracked on the dashboard.

PHASE 2 — EXECUTE (one step at a time, verify EVERY step):
  For each step in your plan:
    1. Execute ONE action (write, run, click, etc.)
    2. EVERY tool response includes a "verification" field and "nextStep" hint — YOU MUST READ THEM
    3. The "verification" field contains proof of what happened:
       - For file writes: actual file content read back automatically (status, lineCount, preview)
       - For commands: stdout, stderr, exitCode surfaced at top level
       - For UI chains: screenshotUrl attached automatically when UI was touched
    4. State out loud: "Step N result: [PASS/FAIL] because [evidence from verification]"
    5. If PASS: call GET ${relayBase}/api/grok-goal?action=advance to mark step done, then proceed
    6. If FAIL: read the actual error, diagnose WHY it failed, change your approach, retry (max 3)
    7. If 3 retries fail: call GET ${relayBase}/api/grok-goal?action=fail, stop, report what went wrong

  CRITICAL: Do NOT proceed to the next step until you have confirmed the current step worked.
  Do NOT assume success — the verification field tells you what actually happened.

PHASE 3 — VERIFY (after all steps complete):
  1. Take a final screenshot: GET ${relayBase}/api/grok-do?chain=screenshot
  2. Summarize what was done and the visible result
  3. Crystallize working knowledge: POST ${relayBase}/api/grok-memory?action=crystallize with body {name, domain, steps}

=== STRUCTURED ERROR DIAGNOSIS ===
  When any step fails:
  1. READ the actual error message from verification.hint or stderr — do not guess
  2. DIAGNOSE: identify the root cause (wrong path? missing dep? wrong coordinates? permission?)
  3. PLAN FIX: state what you'll change and why it should work
  4. RETRY with the different approach
  Never retry the same exact action. Each retry must change something specific.

=== ANTI-PATTERNS (never do these) ===
  - NEVER skip orient — you MUST call grok-orient before doing anything else
  - NEVER ignore the verification field — it contains proof of success/failure
  - NEVER guess what happened — always read actual output/screenshots/verification
  - NEVER proceed after failure without reading and diagnosing the error
  - NEVER repeat a failed approach without changing something specific
  - NEVER execute long sequences blindly — check after each step
  - NEVER assume a UI element is where you expect — screenshot first, then click
  - NEVER send the same request twice — wait for each response
`;
  t = t.replace(/=== RULES ===/, methodologyBlock + '\n=== RULES ===');
  if (!t.includes('CORE METHODOLOGY')) {
    t = methodologyBlock + '\n' + t;
  }
  if (allTunnelUrls.length > 1) {
    const failoverBlock = `\n=== FAILOVER TUNNEL URLS ===\nIf the primary URL above fails or times out, use these alternate bridge URLs (all point to the same desktop):\n${allTunnelUrls.map((u, i) => `  ${i === 0 ? '★ PRIMARY' : `  BACKUP ${i}`}: ${u}`).join('\n')}\nSwitch to the next URL on any network error or timeout. All endpoints work identically on every URL.\n`;
    t = t.replace(/=== END ===$/, failoverBlock + '\n=== END ===');
    if (!t.includes('FAILOVER TUNNEL')) {
      t += '\n' + failoverBlock;
    }
  }
  return t;
  ...
  try {
    ...
    if (isElectronEnv) {
      try {
        ...
        const localStatus = await ipcRenderer.invoke('local-relay-status').catch(() => null);
        if (localStatus?.tunnelUrl) {
          const tunnelBase = localStatus.tunnelUrl.replace(/\/$/, '');
          return {
            snapUrl: `${tunnelBase}/api/snapshot/${project || 'PROJECT_NAME'}`,
            cmdUrl: `${tunnelBase}/api/sandbox/execute`,
            proxyUrl: `${tunnelBase}/api/grok-proxy`,
            editUrl: `${tunnelBase}/api/grok-edit`,
            online: true,
          };
        }
        if (localStatus?.running && localStatus?.localUrl) {
          const localBase = localStatus.localUrl.replace(/\/$/, '');
          return {
            snapUrl: `${localBase}/api/snapshot/${project || 'PROJECT_NAME'}`,
            cmdUrl: `${localBase}/api/sandbox/execute`,
            proxyUrl: `${localBase}/api/grok-proxy`,
            editUrl: `${localBase}/api/grok-edit`,
            online: true,
          };
        }
      } catch {}
    ...
    if (savedMode === 'local' && relayData?.prodRelayUrl) {
...
function buildSandboxApiSection(snapshotUrl: string, cmdEndpoint: string, project: string, bridgeOnline = true, proxyUrl = '', editUrl = ''): string {
  ...
  if (grokDocsUrl) {
    section += `  GET ${grokDocsUrl}           → legacy docs page\n`;
  }
  section += `  GET ${relayBase}/api/diag?project=${proj}\n`;
  section += `  Returns: connection status, roundtrip latency, pass/fail for ALL endpoints, and readyUrls with pre-filled example URLs.\n`;
  section += `  The readyUrls object gives you copy-pasteable URLs for every endpoint — just fill in your params.\n`;
  section += `  Also shows the coord message board size and desktop connection state.\n\n`;

  if (snapshotUrl) {
    section += `READ THE PROJECT (legacy full snapshot — still works but prefer grok-read for individual files):\n`;
  ...
  section += `=== SMART BRIDGE ENDPOINTS (PREFERRED — simple GET, no encoding needed) ===\n`;
  section += `All responses include _bridge metadata with readyUrls and connection state.\n\n`;

  section += `READ FILES — grok-read:\n`;
  section += `  Single file:   GET ${relayBase}/api/grok-read?project=${proj}&path=src/App.tsx\n`;
  ...
  section += `FILE TREE — grok-tree:\n`;
  section += `  Full tree:     GET ${relayBase}/api/grok-tree?project=${proj}\n`;
  section += `  Filtered:      GET ${relayBase}/api/grok-tree?project=${proj}&filter=src\n\n`;
  ...
  section += `  GET ${relayBase}/api/grok-process?project=${proj}&action=start&cmd=npm run dev\n`;
  section += `  GET ${relayBase}/api/grok-process?project=${proj}&action=kill&pid=12345\n`;
  section += `  Actions: list, start, stop, kill, restart, ports, env\n\n`;

  section += `SEARCH — grok-search:\n`;
  section += `  Text/regex:    GET ${relayBase}/api/grok-search?project=${proj}&q=TODO\n`;
  section += `  Symbol search: GET ${relayBase}/api/grok-search?project=${proj}&q=MyComponent&type=symbol\n`;
  section += `  Returns matches with file, line number, and matched text.\n\n`;

  section += `RUN COMMANDS — grok-run:\n`;
  section += `  GET ${relayBase}/api/grok-run?project=${proj}&cmd=npm run build\n`;
  section += `  Returns: { success, results: [{ data: { stdout, stderr, exitCode } }] }\n`;
  section += `  Note: for commands with quotes, URL-encode the entire cmd value.\n\n`;

  section += `MACROS (multiple operations in a single call):\n`;
  section += `  PROJECT STATUS (tree + package.json + git status + preview URL in 1 call):\n`;
  section += `    GET ${relayBase}/api/grok-macro/project-status?project=${proj}\n`;
  section += `  READ WITH CONTEXT (file content + imports analysis in 1 call):\n`;
  section += `    GET ${relayBase}/api/grok-macro/read-context?project=${proj}&path=src/App.tsx\n\n`;

  section += `SCREENSHOT — SIMPLE URL (NO encoding needed, just browse this URL):\n`;
  section += `  GET ${relayBase}/api/screenshot/${proj}?fullPage=true&waitMs=8000\n`;
  section += `  ALWAYS append ?fullPage=true&waitMs=8000 (or 10000ms for safety).\n`;
  section += `  Returns: { success, results: [{ data: { captured: true, screenshotUrl: "https://files.catbox.moe/abc123.png" } }] }\n`;
  ...
  if (interactBaseUrl) {
    section += `BROWSER INTERACTION — grok-interact:\n`;
    ...
    section += `  Params: selector, text, value, x, y, code, script, functionName, args, screenshot=true, waitAfter, timeout\n`;
    section += `  Examples:\n`;
    section += `    ?project=${proj}&action=click&selector=%23submit-btn\n`;
    section += `    ?project=${proj}&action=type&selector=%23input&text=hello+world\n`;
    section += `    ?project=${proj}&action=evaluate&code=return+document.title\n\n`;
  }

  section += `DESKTOP VISION — desktop-frame (JPEG screenshot — your eyes):\n`;
  section += `  GET ${relayBase}/api/desktop-frame\n`;
  section += `  GET ${relayBase}/api/desktop-frame?source=cdp  (Chrome tab only)\n`;
  section += `  ALWAYS take a desktop-frame after every desktop action to verify results.\n\n`;

  section += `CRYSTALLIZATION — grok-memory (persist working knowledge — MANDATORY):\n`;
  section += `  CREATE: POST ${relayBase}/api/grok-memory?action=crystallize  (body: {name, domain, steps, metadata})\n`;
  section += `  LIST:   GET ${relayBase}/api/grok-memory?action=skills\n`;
  section += `  SEARCH: GET ${relayBase}/api/grok-memory?action=recall&query=X\n`;
  section += `  REPLAY: GET ${relayBase}/api/grok-memory?action=replay&skill=NAME\n`;
  section += `  After EVERY productive session, crystallize working knowledge. Before novel tasks, ALWAYS recall crystals first.\n\n`;

  section += `FILE WRITES TO DESKTOP — remote-update:\n`;
  section += `  POST ${relayBase}/api/remote-update  (body: {file, content} or {file, contentB64})\n`;
  section += `  ALWAYS use remote-update for desktop file writes. Never echo/PowerShell for content.\n\n`;

  section += `CALIBRATED CLICK SYSTEM (135/135, 100% accuracy):\n`;
  section += `  physicalX = CSS_X * 2.5, physicalY = CSS_Y * 2.5 + 150 (Chrome UI offset)\n`;
  section += `  CSS viewport: 1536x760 | Physical: 3840x2160 | Scale: 2.5x\n`;
  section += `  Click chaining: hw.exe click X1 Y1 && hw.exe click X2 Y2 in one grok-run call\n\n`;

  section += `AGENT COORDINATION — coord (message board for multi-agent feedback):\n`;
  section += `  Write a note:  GET ${relayBase}/api/coord?note=YOUR_MESSAGE&from=grok\n`;
  section += `  Read board:    GET ${relayBase}/api/coord\n`;
  section += `  Clear board:   GET ${relayBase}/api/coord?clear=1\n`;
  section += `  Use this to leave notes for the user or other agents, check for incoming instructions, and coordinate work.\n\n`;

  section += `SELF-DIAGNOSTIC — diag:\n`;
  section += `  GET ${relayBase}/api/diag?project=${proj}\n`;
  section += `  Returns: relay status, desktop connection, roundtrip latency, endpoint pass/fail, readyUrls.\n`;
  section += `  Call this FIRST when starting work to confirm everything is live.\n\n`;

  section += `RETRY & ERROR HANDLING (follow strictly):\n`;
  section += `  1. Before any edit call, wait at least 6 seconds.\n`;
  ...
  if (proxyBaseUrl) {
    ...
    section += `    { type: "search_files", project: "${proj}", pattern: "Button" }\n`;
    section += `  SHELL / PROCESS / GIT:\n`;
    section += `    { type: "run_command", project: "${proj}", command: "npm run build" }\n`;
    section += `    { type: "install_deps", project: "${proj}" }\n`;
    ...
    section += `  SCREENSHOT via grok-proxy:\n`;
    section += `    { type: "screenshot_preview", project: "${proj}" }\n`;
    section += `    { type: "screenshot_preview", project: "${proj}", selector: "#hero", fullPage: true, waitMs: 3000 }\n`;
    section += `  BROWSER INTERACTION via grok-proxy:\n`;
  ...
  section += `=== DESKTOP CONTROL (system-level — works without any project open) ===\n`;
  section += `Use project=__system__ for operations that are not tied to a specific project.\n`;
  section += `__system__ runs commands from ~/.guardian-ai/tools/ and has NO command restrictions.\n\n`;

  section += `LAUNCH EXECUTABLES — grok-launch-exe:\n`;
  section += `  GET ${relayBase}/api/grok-launch-exe?project=__system__&path=notepad.exe\n`;
  section += `  GET ${relayBase}/api/grok-launch-exe?project=__system__&path=C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe&args=["--remote-debugging-port=9222","--user-data-dir=C:\\\\Users\\\\Aiden\\\\AppData\\\\Local\\\\Temp\\\\chrome-debug","https://example.com"]\n`;
  section += `  The --user-data-dir flag is CRITICAL — without it Chrome joins an existing instance and ignores the debug port.\n\n`;

  section += `LIST WINDOWS — grok-list-windows:\n`;
  section += `  GET ${relayBase}/api/grok-list-windows?project=__system__\n`;
  section += `  Returns: { windows: [{ title, pid, hwnd }] }\n\n`;

  section += `BRING WINDOW TO FRONT — grok-bring-to-front:\n`;
  section += `  GET ${relayBase}/api/grok-bring-to-front?project=__system__&title=PARTIAL_TITLE\n`;
  section += `  title is a partial case-insensitive match.\n\n`;

  section += `SCREENSHOT WINDOW — grok-screenshot-window:\n`;
  section += `  GET ${relayBase}/api/grok-screenshot-window?project=__system__&title=PARTIAL_TITLE\n`;
  section += `  Returns: { image: "base64 PNG data" }\n\n`;

  section += `GET WINDOW INFO — grok-get-window-info:\n`;
  section += `  GET ${relayBase}/api/grok-get-window-info?project=__system__&title=PARTIAL_TITLE\n`;
  section += `  Returns: { x, y, width, height, pid, title }\n\n`;

  section += `CLICK AT COORDINATES — grok-click-at:\n`;
  section += `  GET ${relayBase}/api/grok-click-at?project=__system__&x=500&y=300&button=left\n`;
  section += `  Use grok-get-window-info first to find window position, then calculate absolute screen coords.\n\n`;

  section += `SEND KEYSTROKES — grok-send-keys:\n`;
  section += `  GET ${relayBase}/api/grok-send-keys?project=__system__&keys=KEYS\n`;
  section += `  SendKeys format: hello (text), {ENTER}, {TAB}, ^c (Ctrl+C), ^v (Ctrl+V), %{F4} (Alt+F4)\n\n`;

  section += `RUN SYSTEM COMMANDS (unrestricted with __system__):\n`;
  section += `  GET ${relayBase}/api/grok-run?project=__system__&cmd=ANY_COMMAND\n`;
  section += `  __system__ bypasses the command whitelist — you can run dir, powershell, tasklist, etc.\n`;
  section += `  Commands run from ~/.guardian-ai/tools/ directory.\n\n`;

  section += `=== CDP — CHROME DEVTOOLS PROTOCOL (precise DOM interaction) ===\n`;
  section += `When Chrome is launched with --remote-debugging-port=9222, use the _cdp.js helper to interact with any tab via DOM selectors.\n`;
  section += `_cdp.js is at: ~/.guardian-ai/tools/_cdp.js (also in the project dir)\n\n`;

  section += `HOW TO USE _cdp.js:\n`;
  section += `  GET ${relayBase}/api/grok-run?project=${proj}&cmd=node _cdp.js "JAVASCRIPT_EXPRESSION"\n`;
  section += `  The script connects to Chrome's debug port, finds the first SoundCloud tab, and evaluates the expression.\n\n`;

  section += `CDP EXAMPLES:\n`;
  section += `  Check page title:    node _cdp.js "document.title"\n`;
  section += `  Navigate:            node _cdp.js "window.location.href='https://example.com';'navigating'"\n`;
  section += `  Find buttons:        node _cdp.js "(function(){var btns=document.querySelectorAll('button,[role=button]');return JSON.stringify({count:btns.length,first:btns[0]?{tag:btns[0].tagName,text:btns[0].textContent.trim().slice(0,50)}:null})})()"\n`;
  section += `  Click by selector:   node _cdp.js "document.querySelector('#play-btn').click();'clicked'"\n`;
  section += `  Type into input:     node _cdp.js "var el=document.querySelector('input');el.value='text';el.dispatchEvent(new Event('input',{bubbles:true}));'typed'"\n`;
  section += `  Read element text:   node _cdp.js "document.querySelector('.status').textContent"\n`;
  section += `  Scroll page:         node _cdp.js "window.scrollBy(0,500);'scrolled'"\n\n`;

  section += `CDP BEST PRACTICES:\n`;
  section += `  - ALWAYS use CDP selectors for clicking instead of screen coordinates — selectors are precise, coordinates are guesswork.\n`;
  section += `  - First query the DOM to find the right selector, then click/interact with it.\n`;
  section += `  - For SoundCloud play buttons: document.querySelector('a.sc-button-play.playButton').click()\n`;
  section += `  - Verify actions by checking document.title or element states after interaction.\n`;
  section += `  - _cdp.js filters for SoundCloud tabs by default. To target other sites, modify the tab filter.\n\n`;

  section += `YOUR WORKFLOW (RECOMMENDED):\n`;
  section += `  1. CALL /api/diag to confirm bridge is live and get readyUrls\n`;
  ...
  section += `DESKTOP INTERACTION WORKFLOW:\n`;
  section += `  1. grok-list-windows to see what's open\n`;
  section += `  2. grok-bring-to-front to focus the target window\n`;
  section += `  3. grok-screenshot-window to see current state\n`;
  section += `  4. Use CDP (_cdp.js) for precise DOM interactions — NEVER guess coordinates\n`;
  section += `  5. grok-click-at / grok-send-keys only when CDP is not available (non-browser windows)\n`;
  section += `  6. grok-screenshot-window again to verify the result\n\n`;
  ...
  section += `  - For desktop interactions, ALWAYS prefer CDP DOM selectors over screen coordinates for accuracy.\n`;
  section += `  - project=__system__ has NO command restrictions — use it for system-level operations.\n`;
...
function ClipboardExtractor({ onApply, onApplyAll, onResponseCaptured, activeProject, onGithubImport, onReplaceRepo, toasterConfig, toasterAvailable, userTask, setUserTask, onGenerateContext, onCopyDesktopPrompt, onCopyGeneralPrompt, onEditContext, onEditTemplate, hasCustomTemplate, contextLoading, projectContext, injectTextRef, onPhase3Complete, isRecording, onToggleRecording, recordingChecking }: { onApply: (filePath: string, code: string, editType?: string, searchCode?: string) => void; onApplyAll?: (blocks: { filePath: string; code: string; editType?: string; searchCode?: string }[]) => void; onResponseCaptured?: (fullResponse: string) => void; activeProject?: string | null; onGithubImport?: (url: string) => void; onReplaceRepo?: (url: string) => void; toasterConfig?: OllamaToasterConfig; toasterAvailable?: boolean; userTask: string; setUserTask: (task: string) => void; onGenerateContext: (task?: string) => Promise<void>; onCopyDesktopPrompt: () => Promise<void>; onCopyGeneralPrompt: () => Promise<void>; onEditContext: () => void; onEditTemplate: () => void; hasCustomTemplate: boolean; contextLoading: boolean; projectContext: string; injectTextRef?: React.MutableRefObject<((text: string) => void) | null>; onPhase3Complete?: (convo: Conversation) => void; isRecording?: boolean; onToggleRecording?: () => void; recordingChecking?: boolean }) {
    ...
    <div className={`border-t bg-background/95 backdrop-blur-sm shadow-2xl transition-colors z-20 ${flash ? 'border-primary bg-primary/10' : 'border-primary/30'}`}>
      ...
      <div className="px-4 py-2 flex items-center gap-3 border-b border-border/30 flex-wrap">
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          <button
            onClick={() => onCopyDesktopPrompt()}
            data-testid="button-copy-project-prompt"
            className="flex items-center gap-1 px-2 py-1 rounded text-[9px] bg-cyan-500/15 text-cyan-400 hover:bg-cyan-500/25 transition-colors border border-cyan-500/20 shrink-0 whitespace-nowrap"
            title="Copy project-specific prompt with bridge URL filled in — paste into Grok chat"
          >
            <Monitor className="w-3 h-3" /> Project Prompt
          </button>
          <button
            onClick={() => onCopyGeneralPrompt()}
            data-testid="button-copy-general-prompt"
            className="flex items-center gap-1 px-2 py-1 rounded text-[9px] bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500/25 transition-colors border border-emerald-500/20 shrink-0 whitespace-nowrap"
            title="Copy general desktop control prompt — works for any task, paste into Grok chat"
          >
            <Globe className="w-3 h-3" /> General Prompt
          </button>
          {onToggleRecording && (
            <button
              onClick={onToggleRecording}
              disabled={recordingChecking}
              data-testid="button-toggle-recording"
              className={`flex items-center gap-1 px-2 py-1 rounded text-[9px] transition-colors border shrink-0 whitespace-nowrap ${
                isRecording
              ...
              title={isRecording ? 'Stop recording — saves session with all captured input' : 'Start recording all desktop input (mouse, keys, drags, scroll) for Crystal learning'}
            >
              {recordingChecking ? <Loader2 className="w-3 h-3 animate-spin" /> : isRecording ? <X className="w-3 h-3" /> : <Play className="w-3 h-3" />}
...
interface GrokDesktopBrowserProps {
  ...
  localRelayStatus?: { running: boolean; port: number; localUrl: string | null; tunnelUrl: string | null; tunnelActive: boolean } | null;
}

function GrokDesktopBrowser({ browserUrl, setBrowserUrl, customUrl, setCustomUrl, localRelayStatus }: GrokDesktopBrowserProps) {
  ...
  const currentSite = BROWSER_SITES.find(s => {
    const resolvedUrl = s.id === 'relay' ? (localRelayStatus?.localUrl || s.url) : s.url;
    return browserUrl.startsWith(resolvedUrl);
  });
        ...
        <div className="shrink-0 border-b border-border/30 bg-card/40 px-3 py-2 flex items-center gap-2">
          <div className="flex items-center gap-1 flex-1 overflow-x-auto">
            {BROWSER_SITES.map(site => {
              const siteUrl = site.id === 'relay'
                ? (localRelayStatus?.localUrl || site.url)
                : site.url;
              return (
              <button
                key={site.id}
        ...
        <div className="flex items-center gap-1 flex-1 overflow-x-auto">
          {BROWSER_SITES.map(site => {
            const siteUrl = site.id === 'relay'
              ? (localRelayStatus?.localUrl || site.url)
              : site.url;
            const isActive = browserUrl.startsWith(siteUrl);
            return (
            <button
              key={site.id}
              data-testid={`button-open-${site.id}`}
              onClick={() => navigateTo(siteUrl)}
...
const GrokBridge: React.FC = () => {
  ...
  const [bridgeMode, setBridgeMode] = useState<'dev' | 'local'>(() => {
    try { const saved = localStorage.getItem('lamby-bridge-mode'); return (saved === 'dev' ? 'dev' : 'local') as 'dev' | 'local'; } catch { return 'local'; }
  });
  const [serverDevRelayUrl, setServerDevRelayUrl] = useState<string>('');
  const [localRelayStatus, setLocalRelayStatus] = useState<{ running: boolean; port: number; localUrl: string | null; tunnelUrl: string | null; tunnelActive: boolean; tunnelPool?: { type: string; name?: string; uuid?: string; url: string; alive?: boolean }[]; tunnelPoolSize?: number; tunnelsDir?: string } | null>(null);
  const [tunnelCopied, setTunnelCopied] = useState<string | null>(null);
  ...
  const handleSendLogsToGrok = useCallback(async (formattedPrompt: string) => {
    ...
    if (freshLogs.cmdUrl) {
      enrichedPrompt += `\nYOU HAVE FULL READ/WRITE ACCESS to this project via the Smart Bridge endpoints above.\n`;
      enrichedPrompt += `Use them to investigate and fix the errors shown in these logs:\n`;
      enrichedPrompt += `1. Call /api/diag to confirm bridge is live\n`;
      enrichedPrompt += `2. Use grok-read to read the files referenced in error traces\n`;
      enrichedPrompt += `3. Use grok-search to grep for the broken symbol if needed\n`;
      enrichedPrompt += `4. Use grok-write to apply the fix (auto-verifies the edit landed)\n`;
      enrichedPrompt += `5. Use grok-run to verify the fix compiles (e.g. npm run build)\n`;
      enrichedPrompt += `6. Check console logs to confirm the fix\n`;
      enrichedPrompt += `7. Post a note to /api/coord confirming what you fixed\n`;
      enrichedPrompt += `Prefer the smart endpoints (grok-read, grok-write, grok-search, grok-run) over grok-proxy.\n`;
    }
    try {
      if (isElectron) {
        const { clipboard } = (window as any).require('electron');
  ...
  const handleEditorSendToGrok = useCallback(async (prompt: string) => {
    ...
    if (freshEd.cmdUrl) {
      enriched += `\nUse the Smart Bridge endpoints above to read, modify, and write files directly.\n`;
      enriched += `Prefer grok-read/grok-write/grok-search over grok-proxy — they're simpler (no base64 encoding).\n`;
      enriched += `After making changes, use grok-git to commit and /api/coord to report what you did.\n`;
    }
    if (mode === 'api') {
      setInput(enriched);
  ...
  const sendMessage = useCallback(async () => {
    ...
    if (useFunctionCalling) {
      ...
      await streamGrokFC({
        ...
        systemPrompt: `You are Grok, an autonomous AI agent with full control of a Windows desktop (3840x2160, DPI 240, scale 2.5x) via the Lamby bridge relay. Active project: "${activeProject}". Tools return results directly — do NOT browse URLs. Bridge relay base: ${relayBase}

=== CORE METHODOLOGY (follow this EXACTLY for EVERY task) ===

PHASE 1 — ORIENT (mandatory first step, no exceptions):
  Call: grok-orient with project="${activeProject || 'PROJECT'}" and goal="YOUR_TASK_DESCRIPTION"
  This returns: file tree, git status, git diff, recent commits, open windows, screenshot, relevant skills, recent errors, and all endpoint URLs.
  READ the entire response. Understand the current state BEFORE planning anything.
  The orient call also registers your goal in the step tracker.
  Then: POST to grok-goal with {"goal": "YOUR_GOAL", "steps": ["step1", "step2", ...]}
  This registers your detailed plan so progress is tracked on the dashboard.
...
PHASE 2 — EXECUTE (one step at a time, verify EVERY step):
  For each step in your plan:
    ...
    3. The "verification" field contains proof of what happened:
       ...
       - For UI chains: screenshotUrl attached automatically when UI was touched
    4. State out loud: "Step N result: [PASS/FAIL] because [evidence from verification]"
    5. If PASS: call grok-goal with action=advance to mark step done, then proceed to next step
    6. If FAIL: read the actual error, diagnose WHY it failed, change your approach, retry (max 3)
    7. If 3 retries fail: call grok-goal with action=fail, stop, report what went wrong

  CRITICAL: Do NOT proceed to the next step until you have confirmed the current step worked.
  Do NOT assume success — the verification field tells you what actually happened.

PHASE 3 — VERIFY (after all steps complete):
  1. Take a final screenshot: grok-do with chain=screenshot
  2. Summarize what was done and the visible result
  3. Crystallize working knowledge: POST grok-memory with action=crystallize and body {name, domain, steps}
...
=== CALIBRATED CLICK SYSTEM (135/135, 100% accuracy) ===
  physicalX = CSS_X * 2.5
  physicalY = CSS_Y * 2.5 + 150 (Chrome UI offset)
  CSS viewport: 1536x760. Click chaining: hw.exe click X1 Y1 && hw.exe click X2 Y2

=== DESKTOP CONTROL ===
  project=__system__ for system-level operations.
  Launch executables (grok-launch-exe), list/manage windows (grok-list-windows, grok-bring-to-front, grok-screenshot-window), click at coordinates (grok-click-at), send keystrokes (grok-send-keys), run any command (grok-run with project=__system__).
  SEE the desktop: desktop-frame returns JPEG screenshot.
  Push files: remote-update (POST with {file, content}).

=== CDP (Chrome DevTools Protocol) ===
  When Chrome is running with --remote-debugging-port=9222:
  grok-run with cmd=node _cdp.js "JAVASCRIPT_EXPRESSION"
  ALWAYS prefer CDP selectors over screen coordinates.

=== CRYSTALLIZATION (persist working knowledge) ===
  After EVERY productive session, crystallize:
  CREATE: POST /api/grok-memory?action=crystallize (body: {name, domain, steps, metadata})
  LIST: GET /api/grok-memory?action=skills
  SEARCH: GET /api/grok-memory?action=recall&query=X
  REPLAY: GET /api/grok-memory?action=replay&skill=NAME
  ALWAYS recall crystals before novel tasks.

=== ERROR RECOVERY ===
  On failure: 1) Read the actual error from verification field 2) Diagnose WHY it failed 3) Try a different approach
  On timeout: Switch to a backup tunnel URL if available
  On crash: Restart the application, verify it's running, then continue from last known good state
  Keep a mental checklist: what worked, what failed, what to try next

=== NO-REPLY PROTOCOL ===
  When no-reply mode is active (.local/no-reply-mode.flag = on): ZERO conversational text, output = tool calls ONLY, final output = JSON {"done":true,"actions":[...],"issues":[]}, errors retry 3x silently then structured report.

=== BANNED ===
  grok-3-mini (use grok-4), EEVEE background renders (crash), CompositorNodeGlare (crash).`,
  ...
  const toggleRecording = useCallback(async () => {
    ...
    try {
      const tunnelUrl = localRelayStatus?.tunnelUrl || localRelayStatus?.localUrl || '';
      if (!tunnelUrl) { setStatusMessage('No bridge connection — cannot control recorder'); return; }
      const action = isRecording ? 'stop' : 'start';
      const resp = await fetch(`${tunnelUrl}/api/action-recorder?action=${action}`);
      const data = await resp.json();
      if (data.error) { setStatusMessage('Recorder error: ' + data.error); return; }
      setIsRecording(action === 'start');
      setStatusMessage(action === 'start' ? 'Recording started — do anything on desktop, then click Stop Rec' : 'Recording stopped — session saved');
    } catch (err) {
      setStatusMessage('Failed to toggle recorder: ' + (err instanceof Error ? err.message : String(err)));
    } finally {
      setRecordingChecking(false);
    }
  }, [isRecording, localRelayStatus, setStatusMessage]);
  ...
  const pollLocalRelayStatus = useCallback(async () => {
    if (!isElectron) return;
    try {
      const ipcRenderer = (window as any).require('electron').ipcRenderer;
      const status = await ipcRenderer.invoke('local-relay-status');
      setLocalRelayStatus(status);
    } catch {
      try {
        const res = await fetch('http://localhost:4999/api/bridge-relay-status');
        if (res.ok) {
          const data = await res.json();
          setLocalRelayStatus({
            running: data.usingLocalRelay || false,
            port: data.localRelayPort || 4100,
            localUrl: data.usingLocalRelay ? `http://localhost:${data.localRelayPort || 4100}` : null,
            tunnelUrl: null,
            tunnelActive: false,
          });
        }
  ...
  useEffect(() => {
    async function fetchSnapshotInfo() {
      ...
      if (savedMode === 'local' && relayData?.prodRelayUrl) {
        const localBase = relayData.prodRelayUrl.replace(/^wss:/, 'https:').replace(/^ws:/, 'http:').replace(/\/$/, '');
        setBridgeRelayUrl(relayData.prodRelayUrl);
        setBridgeRelayInput(relayData.prodRelayUrl);
        setSnapshotUrl(`${localBase}/api/snapshot/${activeProject || 'PROJECT_NAME'}`);
        setCommandEndpoint(`${localBase}/api/sandbox/execute`);
        setExternalSnapshotUrl(`${localBase}/api/snapshot/${activeProject || 'PROJECT_NAME'}`);
        setExternalCommandEndpoint(`${localBase}/api/sandbox/execute`);
        try {
          const statusCheck = await fetch(`${localBase}/api/bridge-status`).catch(() => null);
          const statusData = statusCheck?.ok ? await statusCheck.json().catch(() => null) : null;
  ...
  useEffect(() => {
    if (isElectron) {
      ...
      pollLocalRelayStatus();
      let tunnelListener: any = null;
      let tunnelPoolListener: any = null;
      try {
        const ipcRenderer = (window as any).require('electron').ipcRenderer;
        tunnelListener = (_event: any, url: string) => {
          setLocalRelayStatus(prev => prev ? { ...prev, tunnelUrl: url, tunnelActive: true } : { running: true, port: 4100, localUrl: 'http://localhost:4100', tunnelUrl: url, tunnelActive: true });
        };
        tunnelPoolListener = (_event: any, pool: { type: string; name?: string; uuid?: string; url: string }[]) => {
          setLocalRelayStatus(prev => {
            const primary = pool.length > 0 ? pool[0].url : (prev?.tunnelUrl || null);
            return prev
              ? { ...prev, tunnelUrl: primary, tunnelActive: pool.length > 0, tunnelPool: pool, tunnelPoolSize: pool.length }
              : { running: true, port: 4100, localUrl: 'http://localhost:4100', tunnelUrl: primary, tunnelActive: pool.length > 0, tunnelPool: pool, tunnelPoolSize: pool.length };
          });
        };
        ipcRenderer.on('tunnel-url-changed', tunnelListener);
        ipcRenderer.on('tunnel-pool-changed', tunnelPoolListener);
      } catch {}
      return () => {
        if (pollDesktopBridgeStatusRef.current) {
          clearInterval(pollDesktopBridgeStatusRef.current);
        ...
        try {
          const ipcRenderer = (window as any).require('electron').ipcRenderer;
          if (tunnelListener) ipcRenderer.removeListener('tunnel-url-changed', tunnelListener);
          if (tunnelPoolListener) ipcRenderer.removeListener('tunnel-pool-changed', tunnelPoolListener);
        } catch {}
      };
    }
  }, [pollDesktopBridgeStatus, pollLocalRelayStatus]);
                  ...
                  <div className="border-t border-border pt-2 mt-2">
                    ...
                    <div className="flex items-center gap-1" data-testid="bridge-mode-toggle">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          const localWs = 'ws://localhost:4100/bridge-ws';
                          const localBase = 'http://localhost:4100';
                          setBridgeMode('local');
                          try { localStorage.setItem('lamby-bridge-mode', 'local'); } catch {}
                          setBridgeRelayUrl(localWs);
                          setSnapshotUrl(`${localBase}/api/snapshot/${activeProject || 'PROJECT_NAME'}`);
                          setCommandEndpoint(`${localBase}/api/sandbox/execute`);
                          setExternalSnapshotUrl(`${localBase}/api/snapshot/${activeProject || 'PROJECT_NAME'}`);
                          setExternalCommandEndpoint(`${localBase}/api/sandbox/execute`);
                          if (isElectron) {
                            ...
                            setBridgeStatus('connecting');
                            try {
                              await fetch('http://localhost:4999/api/bridge-reconnect', { method: 'POST' });
                            } catch (e) { console.log('[Bridge] bridge-reconnect failed:', e); }
                            pollDesktopBridgeStatus();
                          } else {
                            connectToBridge(localWs, activeProject || 'default');
                          }
                        }}
                        className={`flex-1 px-2 py-1 rounded text-[9px] border transition-colors ${
                          bridgeMode === 'local'
                    ...
                    <p className="text-muted-foreground/60 mt-1">
                      {bridgeMode === 'dev' ? `Dev: ${serverDevRelayUrl || 'dev relay'}` : 'Local: ws://localhost:4100'}
                    </p>
                  </div>
                  {isElectron && localRelayStatus && (
                    <div className="border-t border-border pt-2 mt-2 space-y-1.5">
                      <div className="text-muted-foreground font-medium flex items-center gap-1.5">
                        Local Relay
                        <span className={`inline-block w-1.5 h-1.5 rounded-full ${localRelayStatus.running ? 'bg-green-500' : 'bg-red-500'}`} />
                      </div>
                      <div className="text-[10px] text-muted-foreground/80 space-y-0.5">
                        <div>Port {localRelayStatus.port}: {localRelayStatus.running ? 'Running' : 'Stopped'}</div>
                        {localRelayStatus.localUrl && <div>Local: {localRelayStatus.localUrl}</div>}
                      </div>
                      {localRelayStatus.tunnelPool && localRelayStatus.tunnelPool.length > 0 ? (
                        <div className="space-y-1">
                          <div className="text-[9px] text-muted-foreground/60">Tunnel Pool ({localRelayStatus.tunnelPool.filter(t => t.url).length}/{localRelayStatus.tunnelPool.length} active)</div>
                          {localRelayStatus.tunnelPool.map((t, i) => (
                            <div key={i} className="flex items-center gap-1">
                              <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${t.url ? 'bg-green-500' : t.alive ? 'bg-yellow-500 animate-pulse' : 'bg-red-500'}`} />
                              <div className={`flex-1 px-2 py-0.5 rounded border text-[9px] truncate ${t.url ? 'border-green-500/30 bg-green-500/5 text-green-400' : 'border-border/30 bg-muted/30 text-muted-foreground/50'}`} data-testid={`text-tunnel-url-${i}`} title={t.url || `${t.name} — connecting...`}>
                                {t.url || `${t.name} — connecting...`}
                              </div>
                              {t.url && (
                                <button
                                  data-testid={`button-copy-tunnel-url-${i}`}
                                  onClick={() => {
                                    navigator.clipboard.writeText(t.url!);
                                    setTunnelCopied(t.url!);
                                    setTimeout(() => setTunnelCopied(null), 2000);
                                  }}
                                  className="px-1 py-0.5 rounded text-[8px] bg-green-500/10 text-green-400 hover:bg-green-500/20 border border-green-500/20 shrink-0"
                                  title={`Copy ${t.name} URL`}
                                >
                                  {tunnelCopied === t.url ? <ClipboardCheck className="w-2.5 h-2.5" /> : <Clipboard className="w-2.5 h-2.5" />}
                                </button>
                              )}
                            </div>
                          ))}
                        </div>
                      ) : localRelayStatus.tunnelUrl ? (
                        <div className="flex items-center gap-1">
                          <div className="flex-1 px-2 py-1 rounded border border-green-500/30 bg-green-500/5 text-green-400 text-[10px] truncate" data-testid="text-tunnel-url" title={localRelayStatus.tunnelUrl}>
                            {localRelayStatus.tunnelUrl}
                          </div>
                          <button
                            data-testid="button-copy-tunnel-url"
                            onClick={() => {
                              navigator.clipboard.writeText(localRelayStatus.tunnelUrl || '');
                              setTunnelCopied(localRelayStatus.tunnelUrl || '');
                              setTimeout(() => setTunnelCopied(null), 2000);
                            }}
                            className="px-1.5 py-1 rounded text-[9px] bg-green-500/10 text-green-400 hover:bg-green-500/20 border border-green-500/20"
                            title="Copy tunnel URL for Grok"
                          >
                            {tunnelCopied ? <ClipboardCheck className="w-3 h-3" /> : <Clipboard className="w-3 h-3" />}
                          </button>
                        </div>
                      ) : localRelayStatus.running ? (
                        <div className="text-[10px] text-yellow-500/80 flex items-center gap-1">
                          <Loader2 className="w-3 h-3 animate-spin" /> Starting 3 tunnels...
                        </div>
                      ) : null}
                      <button
                        data-testid="button-restart-local-relay"
                        onClick={async () => {
                          try {
                            const ipcRenderer = (window as any).require('electron').ipcRenderer;
                            await ipcRenderer.invoke('restart-local-relay');
                            setStatusMessage('Local relay restarting...');
                            setTimeout(() => pollLocalRelayStatus(), 3000);
                          } catch (e: any) { setStatusMessage(`Error: ${e.message}`); }
                        }}
                        className="w-full px-2 py-1 rounded text-[9px] bg-muted/50 text-muted-foreground hover:bg-muted border border-border/30"
                      >
                        <RefreshCw className="w-3 h-3 inline mr-1" /> Restart Local Relay
                      </button>
                    </div>
                  )}
                  {isElectron && (
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden">
        ...
        <div ref={innerRowRef} className="flex-1 flex min-h-0 overflow-hidden">
          ...
          <div className="flex-1 min-w-0 flex flex-col min-h-0 overflow-y-auto" style={showPreviewEmbed && previewPort ? { flex: '1 1 50%' } : undefined}>
            {mode === 'browser' && (
              <GrokDesktopBrowser browserUrl={browserUrl} setBrowserUrl={setBrowserUrl} customUrl={customUrl} setCustomUrl={setCustomUrl} localRelayStatus={localRelayStatus} />
            )}
            ...
            <ParallaxPortal wall="bottom">
              <ClipboardExtractor onApply={applyBlock} onApplyAll={batchApplyAll} onResponseCaptured={(text) => { lastFullResponseRef.current = text; }} activeProject={activeProject} onGithubImport={handleGitHubImport} onReplaceRepo={handleReplaceRepo} toasterConfig={toasterConfig} toasterAvailable={toasterAvailability?.available} userTask={userTask} setUserTask={setUserTask} onGenerateContext={async (task?: string) => { const ctx = await buildProjectContext(task); if (ctx) copyContextToClipboard(ctx); }} onCopyDesktopPrompt={async () => {
                try {
                  const freshCtx = await fetchFreshBridgeEndpoints(activeProject || '');
                  const allTunnelUrls = (localRelayStatus?.tunnelPool || []).filter(t => t.url).map(t => t.url!);
                  const relayBase = freshCtx.online && freshCtx.snapUrl ? freshCtx.snapUrl.replace(/\/api\/snapshot\/.*$/, '') : (allTunnelUrls[0] || localRelayStatus?.tunnelUrl || localRelayStatus?.localUrl || '');
                  const resp = await fetch('/grok-desktop-prompt.txt');
                  if (!resp.ok) { setStatusMessage('Could not load desktop prompt template'); return; }
                  let template = await resp.text();
                  template = injectMethodologyAndFailover(template, relayBase, allTunnelUrls);
                  ...
                  setStatusMessage(`Project prompt copied with ${allTunnelUrls.length} tunnel URL${allTunnelUrls.length !== 1 ? 's' : ''} — paste into Grok chat`);
                } catch (err) {
                  setStatusMessage('Could not copy project prompt');
                }
              }} onCopyGeneralPrompt={async () => {
                try {
                  const freshCtx = await fetchFreshBridgeEndpoints(activeProject || '');
                  const allTunnelUrls = (localRelayStatus?.tunnelPool || []).filter(t => t.url).map(t => t.url!);
                  const relayBase = freshCtx.online && freshCtx.snapUrl ? freshCtx.snapUrl.replace(/\/api\/snapshot\/.*$/, '') : (allTunnelUrls[0] || localRelayStatus?.tunnelUrl || localRelayStatus?.localUrl || '');
                  const resp = await fetch('/grok-general-prompt.txt');
                  if (!resp.ok) { setStatusMessage('Could not load general prompt'); return; }
                  let template = await resp.text();
                  template = injectMethodologyAndFailover(template, relayBase, allTunnelUrls);
                  ...
                  const a = document.createElement('a');
                  a.href = dlUrl;
                  a.download = 'grok-general-prompt.txt';
                  document.body.appendChild(a);
                  a.click();
                  document.body.removeChild(a);
