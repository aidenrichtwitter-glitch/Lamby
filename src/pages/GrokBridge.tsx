...
function buildSandboxApiSection(snapshotUrl: string, cmdEndpoint: string, project: string, bridgeOnline = true): string {
  ...
  if (cmdEndpoint) {
    ...
    section += `    { type: "read_multiple_files", project: "${proj}", paths: ["src/App.tsx","src/main.ts"] }  → batch read\n`;
    ...
    section += `    { type: "bulk_write", project: "${proj}", files: [{path:"a.ts",content:"..."},{path:"b.ts",content:"..."}] }  → atomic multi-file write (rollback on failure)\n`;
    section += `    { type: "delete_file", project: "${proj}", path: "src/old.ts" }\n`;
    section += `    { type: "bulk_delete", project: "${proj}", paths: ["src/old.ts","src/tmp/"] }  → delete multiple files/folders\n`;
    ...
    section += `    { type: "copy_folder", project: "${proj}", source: "src/components", dest: "src/components-backup" }  → recursive copy\n`;
    ...
    section += `    { type: "search_replace", project: "${proj}", path: "src/App.tsx", search: "oldText", replace: "newText" }  → find & replace (regex flag optional)\n`;
    section += `    { type: "search_replace", project: "${proj}", paths: ["src/a.ts","src/b.ts"], search: "old", replace: "new", regex: true }\n`;
    section += `    { type: "apply_patch", project: "${proj}", patch: "--- a/file\\n+++ b/file\\n@@ ...\\n-old\\n+new" }  → apply unified diff\n`;
    ...
    section += `    { type: "add_dependency", project: "${proj}", name: "lodash", dev: false }  → add package (optional version, dev flag)\n`;
    section += `  CODE QUALITY:\n`;
    section += `    { type: "type_check", project: "${proj}" }  → runs tsc --noEmit\n`;
    section += `    { type: "lint_and_fix", project: "${proj}" }  → eslint --fix or prettier\n`;
    section += `    { type: "format_files", project: "${proj}", files: "src/" }  → prettier --write\n`;
    section += `  DEV SERVER & PROCESS:\n`;
    section += `    { type: "start_process", project: "${proj}", command: "npm run dev" }\n`;
    section += `    { type: "list_processes", project: "${proj}" }\n`;
    section += `    { type: "kill_process", project: "${proj}", name: "proc-name" }\n`;
    section += `    { type: "restart_dev_server", project: "${proj}", command: "npm run dev" }  → kills all, optionally restarts\n`;
    section += `    { type: "list_open_ports", project: "${proj}" }  → shows listening ports\n`;
    section += `  GIT:\n`;
    ...
    section += `    { type: "git_push", project: "${proj}", remote: "origin", branch: "main" }\n`;
    section += `    { type: "git_pull", project: "${proj}", remote: "origin" }\n`;
    section += `    { type: "git_merge", project: "${proj}", branch: "feature-x" }\n`;
    section += `  ENVIRONMENT:\n`;
    section += `    { type: "set_env_var", project: "${proj}", key: "API_KEY", value: "..." }  → sets in .env\n`;
    section += `    { type: "get_env_vars", project: "${proj}" }  → reads .env key-value pairs\n`;
    section += `    { type: "rollback_last_change", project: "${proj}", files: "src/App.tsx" }  → git checkout specific files\n`;
    section += `    { type: "rollback_last_change", project: "${proj}" }  → git stash pop or checkout all\n`;
    ...
    section += `    { type: "tailwind_audit", project: "${proj}" }  → Tailwind config, custom colors, used classes\n`;
    section += `    { type: "find_usages", project: "${proj}", symbol: "MyComponent" }  → grep symbol with context lines\n`;
    section += `    { type: "component_tree", project: "${proj}" }  → React import/export/JSX dependency tree\n`;
    section += `    { type: "extract_theme", project: "${proj}" }  → CSS custom properties + Tailwind color config\n`;
    section += `    { type: "extract_colors", project: "${proj}" }  → alias for extract_theme\n`;
    section += `  PREVIEW:\n`;
    section += `    { type: "get_preview_url", project: "${proj}" }  → returns running dev server URL/port\n`;
    section += `    { type: "capture_preview", project: "${proj}" }  → preview URL + screenshot info\n`;
    section += `  AI GENERATION (requires XAI_API env var):\n`;
    section += `    { type: "generate_component", project: "${proj}", spec: "a dark-themed login form", name: "LoginForm" }  → AI generates component file\n`;
    section += `    { type: "generate_page", project: "${proj}", spec: "dashboard with charts", name: "Dashboard" }  → AI generates page file\n`;
    section += `    { type: "refactor_file", project: "${proj}", path: "src/App.tsx", instructions: "add error boundary" }  → AI refactors existing file\n`;
    section += `  VALIDATION & PERFORMANCE:\n`;
    section += `    { type: "validate_change", project: "${proj}" }  → runs type-check + lint, reports pass/fail\n`;
    section += `    { type: "profile_performance", project: "${proj}" }  → bundle sizes + lighthouse availability\n`;
    section += `  FOLDER OPERATIONS:\n`;
    section += `    { type: "create_folder", project: "${proj}", path: "src/components/new" }  → create directory recursively\n`;
    section += `    { type: "delete_folder", project: "${proj}", path: "src/old", recursive: true }  → remove folder\n`;
    section += `    { type: "move_folder", project: "${proj}", from: "src/old", to: "src/new" }  → move/rename folder\n`;
    section += `    { type: "rename_folder", project: "${proj}", source: "src/old", newName: "renamed" }  → rename folder in place\n`;
    section += `    { type: "list_tree_filtered", project: "${proj}", filter: "ts|tsx", depth: 4 }  → filtered file listing\n`;
    section += `  CODE INTELLIGENCE:\n`;
    section += `    { type: "dead_code_detection", project: "${proj}" }  → finds exported but unused modules\n`;
    section += `    { type: "dependency_graph", project: "${proj}" }  → import/require graph of all modules\n`;
    section += `    { type: "symbol_search", project: "${proj}", query: "Button" }  → search function/class/type definitions\n`;
    section += `    { type: "grep_advanced", project: "${proj}", pattern: "TODO|FIXME", include: [".ts"], case_sensitive: false }  → regex with file filters\n`;
    section += `    { type: "extract_imports", project: "${proj}", file: "src/App.tsx" }  → list all imports from a file\n`;
    section += `  BUILD & EXECUTION:\n`;
    section += `    { type: "run_command_advanced", project: "${proj}", command: "npm run build", timeout: 60000, env: { NODE_ENV: "production" } }  → shell with timeout/env\n`;
    section += `    { type: "build_with_flags", project: "${proj}", flags: ["--mode", "production"] }  → build with custom flags\n`;
    section += `    { type: "clean_build_cache", project: "${proj}" }  → removes dist, .next, .cache, build, etc.\n`;
    section += `  PROCESS MANAGEMENT:\n`;
    section += `    { type: "start_process_named", project: "${proj}", command: "npm run dev", name: "my-server" }  → start with explicit name\n`;
    section += `    { type: "monitor_process", project: "${proj}", pid: 1234 }  → check if process alive\n`;
    section += `    { type: "get_process_logs", project: "${proj}", name: "dev-server" }  → get stdout/stderr of running process\n`;
    section += `    { type: "stop_all_processes", project: "${proj}" }  → SIGTERM all sandbox processes\n`;
    section += `    { type: "switch_port", project: "${proj}", port: 3001 }  → register port preference\n`;
    section += `  GIT (EXTENDED):\n`;
    section += `    { type: "git_stash_pop", project: "${proj}" }  → pop latest stash\n`;
    section += `    { type: "git_reset", project: "${proj}", mode: "hard", ref: "HEAD~1" }  → git reset (soft/hard)\n`;
    section += `    { type: "git_revert", project: "${proj}", commit: "abc123" }  → revert a commit\n`;
    section += `    { type: "git_tag", project: "${proj}", name: "v1.0.0", message: "Release" }  → create annotated tag\n`;
    section += `  VISUAL & PREVIEW:\n`;
    section += `    { type: "visual_diff", project: "${proj}", beforeUrl: "...", afterUrl: "..." }  → visual comparison stub\n`;
    section += `    { type: "capture_component", project: "${proj}", componentName: "Header" }  → component screenshot stub\n`;
    section += `    { type: "record_video", project: "${proj}", duration: 5 }  → video recording stub\n`;
    section += `    { type: "get_dom_snapshot", project: "${proj}" }  → fetch HTML of running preview\n`;
    section += `    { type: "get_console_errors", project: "${proj}" }  → extract errors from running processes\n`;
    section += `  AI GENERATION (EXTENDED):\n`;
    section += `    { type: "generate_test", project: "${proj}", file: "src/utils.ts" }  → AI generates unit test file\n`;
    section += `    { type: "generate_storybook", project: "${proj}", component: "Button" }  → AI generates Storybook stories\n`;
    section += `    { type: "optimize_code", project: "${proj}", file: "src/App.tsx" }  → AI optimizes file for performance\n`;
    section += `    { type: "convert_to_typescript", project: "${proj}", file: "src/utils.js" }  → AI converts JS to TS\n`;
    section += `    { type: "add_feature", project: "${proj}", featureSpec: "add dark mode toggle", path: "src/ThemeToggle.tsx" }  → AI generates feature\n`;
    section += `    { type: "migrate_framework", project: "${proj}", target: "vite" }  → migration guide for framework switch\n`;
    section += `  DEBUGGING & PROFILING:\n`;
    section += `    { type: "react_profiler", project: "${proj}" }  → static analysis for React perf issues\n`;
    section += `    { type: "memory_leak_detection", project: "${proj}" }  → scan for event/timer/connection leaks\n`;
    section += `    { type: "console_error_analysis", project: "${proj}" }  → categorize errors/warnings from all processes\n`;
    section += `    { type: "runtime_error_trace", project: "${proj}" }  → find error boundaries, try/catch, unhandled throws\n`;
    section += `    { type: "bundle_analyzer", project: "${proj}" }  → detailed build output analysis by file type\n`;
    section += `    { type: "network_monitor", project: "${proj}" }  → list active TCP connections\n`;
    section += `    { type: "accessibility_audit", project: "${proj}" }  → heuristic a11y scan (alt, roles, labels)\n`;
    section += `    { type: "security_scan", project: "${proj}" }  → npm audit + dangerous patterns + hardcoded secrets\n`;
    section += `  CONFIG & META:\n`;
    section += `    { type: "set_tailwind_config", project: "${proj}", config: { content: ["./src/**/*.tsx"] } }  → create/update tailwind config\n`;
    section += `    { type: "set_next_config", project: "${proj}", config: { reactStrictMode: true } }  → create/update next.js config\n`;
    section += `    { type: "update_package_json", project: "${proj}", changes: { scripts: { dev: "vite" } } }  → merge changes into package.json\n`;
    section += `    { type: "manage_scripts", project: "${proj}", scriptName: "lint", command: "eslint src/" }  → add/update/delete npm scripts\n`;
    section += `    { type: "switch_package_manager", project: "${proj}", manager: "pnpm" }  → switch between npm/yarn/pnpm\n`;
    section += `  SUPER & META:\n`;
    section += `    { type: "deploy_preview", project: "${proj}", command: "npm run dev" }  → start preview server\n`;
    section += `    { type: "export_project_zip", project: "${proj}" }  → alias for export_project format:zip\n`;
    section += `    { type: "import_project", project: "${proj}", url: "https://github.com/user/repo", name: "my-project" }  → clone git repo as new project\n`;
    section += `    { type: "super_command", project: "${proj}", description: "add a footer component with social links" }  → AI translates natural language to action list\n\n`;
