...
function responseHasCodeBlocks(text: string): boolean {
  return /```[\s\S]*?```/.test(text) || /\/\/ file:\s/.test(text) || /===\s*DEPENDENCIES\s*===/.test(text);
}

async function ollamaChatRaw(
  prompt: string,
  cfg: OllamaToasterConfig,
...
${truncated}`;
  ...
  const cleaned = await ollamaChatRaw(prompt, cfg);
...
async function extractionPass(
  ...
  const { parseCodeBlocks, parseDependencies, parseActionItems } = await import('./code-parser');
  const truncated = rawResponse.slice(0, 10000);

  const prompt = `You are a code extraction engine for Lamby. The text below is a casual, conversational AI reply that describes code changes, file edits, dependency installs, or shell commands WITHOUT using proper code block formatting.

Your job: extract every actionable item and output them in Lamby's strict format. Output NOTHING else — no greetings, no explanations, no commentary.

FORMAT RULES:
1. For code file changes, output a fenced code block with the file path on the first line:
\`\`\`tsx
...
2. For npm dependencies, output:
=== DEPENDENCIES ===
package-name
dev: dev-package-name
=== END_DEPENDENCIES ===

3. For shell commands, output:
=== COMMANDS ===
npm run build
=== END_COMMANDS ===

4. If the text mentions changing a specific part of a file but doesn't give the full file, output a search/replace block:
\`\`\`tsx
// file: src/App.tsx
<<<<<<< SEARCH
old code here
...
5. If the text is purely conversational with NO actionable code changes at all (just discussion, questions, explanations), output exactly: NO_ACTIONS_FOUND

CASUAL TEXT TO EXTRACT FROM:
${truncated}`;

  const cleaned = await ollamaChatRaw(prompt, cfg, 6144, 90_000);
  if (!cleaned || cleaned.trim() === 'NO_ACTIONS_FOUND') return null;

  const ollamaBlocks = parseCodeBlocks(cleaned);
  const deps = parseDependencies(cleaned);
  const actions = parseActionItems(cleaned);

  const hasDeps = deps.dependencies.length > 0 || deps.devDependencies.length > 0;
  const hasCommands = actions.some(a => a.type === 'command');

  if (ollamaBlocks.length === 0 && !hasDeps && !hasCommands) return null;
  ...
  if (hasDeps) {
    ...
    for (const d of deps.dependencies) unparsed += `${d}\n`;
    for (const d of deps.devDependencies) unparsed += `dev: ${d}\n`;
    unparsed += '=== END_DEPENDENCIES ===\n\n';
  }
  if (hasCommands) {
    unparsed += '=== COMMANDS ===\n';
    for (const a of actions) {
      if (a.type === 'command' && a.command) unparsed += `${a.command}\n`;
    }
    unparsed += '=== END_COMMANDS ===\n';
  }

  const blockCount = ollamaBlocks.length;
  const pathCount = ollamaBlocks.filter(b => b.filePath).length;
  const extras = [
    hasDeps ? `${deps.dependencies.length + deps.devDependencies.length} deps` : '',
    hasCommands ? `${actions.filter(a => a.type === 'command').length} cmds` : '',
  ].filter(Boolean).join(', ');

  return {
    reasoning: `Toaster extracted from casual text → ${blockCount} blocks (${pathCount} with paths)${extras ? `, ${extras}` : ''}`,
    files,
...
export async function cleanGrokResponse(
  ...
  try {
    if (responseHasCodeBlocks(rawResponse)) {
      const result = await reformatPass(rawResponse, cfg);
      if (result && result.files.length > 0) return result;
    }

    const result = await extractionPass(rawResponse, cfg);
    return result;
  } catch {
    return null;
  }
