...
function formatLogsForGrok(logs: LogEntry[], activeProject?: string | null): string {
  const errorWarnings = logs.filter(l => l.level === 'error' || l.level === 'warn');
  const relevantLogs = errorWarnings.length > 0 ? errorWarnings.slice(-20) : logs.slice(-20);

  let prompt = `=== PREVIEW CONSOLE LOGS ===\n`;
  prompt += `Captured from ${activeProject || 'app'} preview iframe.\n`;
  prompt += `Total entries: ${logs.length} (${logs.filter(l => l.level === 'error').length} errors, ${logs.filter(l => l.level === 'warn').length} warnings)\n\n`;
